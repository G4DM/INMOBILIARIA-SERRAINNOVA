          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-[#e7f3eb] dark:divide-[#1a3022]">
              <tr v-for="post in filteredBlogPosts" :key="post.id"
                ...
                <td class="px-6 py-4 text-[#4c9a66] dark:text-[#a0ccb0]">{{ new
                  Date(post.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }) }}
                </td>
                <td class="px-6 py-4">
                  <span v-if="post.hidden" class="text-red-600 dark:text-red-400 font-medium flex items-center gap-1">
                ...
                <td class="px-6 py-4">
                  <div class="flex gap-2">
                    ...
                    <button @click="toggleBlogVisibility(post)"
                      class="px-3 py-1.5 rounded-lg font-medium transition-all text-sm" :class="post.hidden
      ...
      <div
        ...
        <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-bold mb-2">Categoría</label>
              <select v-model="editingBlog.category"
                class="w-full px-4 py-3 rounded-lg border border-[#cfe7d7] dark:border-[#2a4a35] bg-[#f6f8f6] dark:bg-[#102216] focus:ring-2 focus:ring-primary">
                <option value="Estilos de vida">Estilos de vida</option>
                <option value="Agenda 2030">Agenda 2030</option>
                <option value="Subvenciones">Subvenciones</option>
            ...
            <div>
              ...
              <input v-model="editingBlog.date" type="date"
                class="w-full px-4 py-3 rounded-lg border border-[#cfe7d7] dark:border-[#2a4a35] bg-[#f6f8f6] dark:bg-[#102216] focus:ring-2 focus:ring-primary" />
            </div>

            <div class="col-span-2">
              <label class="block text-sm font-bold mb-2">URL de Imagen</label>
              <input v-model="editingBlog.image" type="text"
                class="w-full px-4 py-3 rounded-lg border border-[#cfe7d7] dark:border-[#2a4a35] bg-[#f6f8f6] dark:bg-[#102216] focus:ring-2 focus:ring-primary"
            ...
            <div class="col-span-2">
              ...
              <textarea v-model="editingBlog.excerpt" rows="2"
                class="w-full px-4 py-3 rounded-lg border border-[#cfe7d7] dark:border-[#2a4a35] bg-[#f6f8f6] dark:bg-[#102216] focus:ring-2 focus:ring-primary"
            ...
            <div class="col-span-2">
              ...
              <textarea v-model="editingBlog.content" rows="4"
                class="w-full px-4 py-3 rounded-lg border border-[#cfe7d7] dark:border-[#2a4a35] bg-[#f6f8f6] dark:bg-[#102216] focus:ring-2 focus:ring-primary"
...
<script setup>
  ...
  const filteredBlogPosts = computed(() => {
    return blogPosts.value.filter(post => {
      ...
      const matchesCategory = searchCategory.value === '' || category === searchCategory.value
  ...
  const saveBlogPost = async () => {
    try {
      const url = isEditingBlog.value
        ? `http://localhost:8000/api/articles/${editingBlog.value.id}`
        : 'http://localhost:8000/api/articles'

      const method = isEditingBlog.value ? 'PUT' : 'POST'

      const response = await fetch(url, {
        method: isEditingBlog.value ? 'PUT' : 'POST',
        headers: {
          'Authorization': `Bearer ${userStore.token}`,
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
        body: JSON.stringify(editingBlog.value)
      })

      if (!response.ok) throw new Error('Error al guardar artículo')

      const savedPost = await response.json()

      if (isEditingBlog.value) {
        const index = blogPosts.value.findIndex(p => p.id === savedPost.id)
        if (index !== -1) blogPosts.value[index] = savedPost
      } else {
  ...
  const toggleBlogVisibility = async (post) => {
    try {
      const updatedPost = { ...post, hidden: !post.hidden }
      const response = await fetch(`http://localhost:8000/api/articles/${post.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${userStore.token}`,
          'Content-Type': 'application/json',
      ...
      const index = blogPosts.value.findIndex(p => p.id === data.id)
      if (index !== -1) blogPosts.value[index] = data
    } catch (err) {
      alert(err.message)
    }
  }
